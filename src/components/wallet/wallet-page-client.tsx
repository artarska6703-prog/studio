...
export function WalletPageView({ address }: WalletPageViewProps) {
  ...
  useEffect(() => {
    ...
    if (!useMockData) {
      ...
      setWalletDetails({
        address: address,
        sol: { balance: 1234.56, price: MOCK_SOL_PRICE, valueUSD: 1234.56 * MOCK_SOL_PRICE },
        tokens: [
          {
  ...
  }, [address, useMockData]); // ✅ loop fixed

  // ---- TRANSACTIONS (LIVE/MOCK) ----
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          <TabsContent value="portfolio" className="mt-6 space-y-8">
            <div className="grid gap-8 md:grid-cols-3">
              {walletDetails ? (
                <>
                  <BalanceCard balance={walletDetails.sol.balance} balanceUSD={walletDetails.sol.valueUSD} />
            ...
            {walletDetails ? (
              <PortfolioCompositionChart solValue={walletDetails.sol.valueUSD} tokens={enrichedTokens ?? []} />
            ) : (
              <p>Loading Chart...</p>
            )}
          </TabsContent>
