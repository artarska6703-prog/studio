...
import type { WalletDetails, FlattenedTransaction, TokenHolding, AddressNameAndTags } from "@/lib/types";
...
if (data?.namesAndTags && typeof data.namesAndTags === 'object') {
  setAddressTags((prev) => ({ ...prev, ...data.namesAndTags }));
}
    } catch (e) {
  ...
  const liveTransactions = useMemo((): FlattenedTransaction[] => {
    if (useMockData) {
        switch (mockScenario) {
            case 'whale': return getWhaleTxs(address) as FlattenedTransaction[];
            case 'degen': return getDegenTxs(address) as FlattenedTransaction[];
            case 'balanced': default: return getBalancedTxs(address) as FlattenedTransaction[];
        }
    }
  ...
  const enrichedTokens: TokenHolding[] | undefined = useMemo(() => {
    return walletDetails?.tokens.map(token => {
        ...
        const valueUSD = (isNaN(price) || isNaN(amount)) ? 0 : amount * price;
