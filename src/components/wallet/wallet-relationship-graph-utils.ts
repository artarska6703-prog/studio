...
const getNodeType = (
  ...
  if (tagsAndName && tagsAndName.tags.length > 0) {
    ...
    if (tags.includes("shark")) return "shark";
    if (tags.includes("dolphin")) return "dolphin";
    if (tags.includes("fish")) return "fish";
    if (tags.includes("shrimp")) return "shrimp";
  }

  // Fallback only if no valid tags
  const usd = balanceUSD || 0;
  if (usd > 100000) return "whale";
  if (usd > 50000) return "shark";
  if (usd > 10000) return "dolphin";
  if (usd > 1000 || balance > 100) return "fish";
...
export const processTransactions = (
  ...
  const links = Object.values(allLinks).filter(
    link => finalNodeIds.has(link.from) && finalNodeIds.has(link.to)
  );
